...
class Timer
{
     ...
     * 2025/1/6 09:56
     */
    private static function getDaysArr($days, $start, string $format, string $sort): array
    {
        $arr = [];
        for ($i = 0; $i < $days; $i++) {
            $this_start = $start + $i * 3600 * 24;
            $this_end = $this_start + 3600 * 24;
            $arr[] = [
                'start' => $this_start,
                'end' => $this_end,
                'start_time_format' => date($format, $this_start),
            ];
        }
        //默认正序，非正序则翻转数组
        return $sort == 'asc' ? $arr : array_reverse($arr);
    }
     ...
     * 2025/1/6 09:53
     */
    public static function getBeforeDaysArr($start, $days, bool $about_today=true, string $format='Y-m-d', string $sort='asc'): array
    {
        //获取指定时间前*天的凌晨时间戳
        $get_start_days=$about_today?$days-1:$days;
        if(!self::isTimestamp($start)) {
            $start=self::toTimestamp($start);
        }
        $start=self::beforeDay($get_start_days,$start);
